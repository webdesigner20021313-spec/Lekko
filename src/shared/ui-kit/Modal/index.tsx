...
>(({ className, children, ...props }, ref) => (
  <ModalPortal>
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 rounded-md p-1 text-gray-400 opacity-70 ring-offset-white transition-opacity hover:bg-gray-100 hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-gray-900/20 focus:ring-offset-2 disabled:pointer-events-none">
        <X className="h-4 w-4" />
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn(
      "font-lexend text-lg font-semibold leading-none tracking-tight text-gray-900",
      className
    )}
    {...props}
  />
