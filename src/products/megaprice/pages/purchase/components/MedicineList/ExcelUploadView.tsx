...
export function ExcelUploadView({
  ...
  // ── MAPPING ───────────────────────────────────────────────────────────────
  if (step === 'mapping') {
    // Содержимое — одно и то же для mobile BottomSheet и desktop modal.
    const titleNode = (
      <div className="flex items-center gap-2 min-w-0">
        <FileSpreadsheet className="h-4 w-4 shrink-0 text-gray-400" />
        <span className="truncate text-sm font-semibold text-gray-900 dark:text-gray-100">{fileName}</span>
        <span className="shrink-0 text-xs text-gray-400 dark:text-gray-500">· {t('excel_rows_count', { count: rawData.length - 1 })}</span>
      </div>
    )
        ...
        {(() => {
          ...
          return (
            <div>
              ...
              {/* Таблица с буквами колонок — на мобиле скроллится по горизонтали. */}
    ...
    const applyBtn = (
      <button
        ...
        className="h-12 w-full rounded-xl bg-gray-900 text-sm font-semibold text-white transition-colors hover:bg-black md:h-9 md:rounded-lg dark:bg-[#f1f1f1] dark:text-gray-900 dark:hover:bg-[#e0e0e0] disabled:cursor-not-allowed disabled:opacity-40"
      >
        {t('excel_apply')}
      </button>
    )
    ...
    return (
      <>
        ...
        {/* Desktop: centered modal (как было) */}
        <div className="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-[1px] p-4 md:flex">
          <div className="flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-[#090909] dark:border dark:border-gray-700">
            {/* Modal header */}
            ...
            {/* Modal footer */}
            <div className="shrink-0 border-t border-gray-200 bg-white px-5 py-4 dark:border-gray-700 dark:bg-[#090909]">
              {applyBtn}
            </div>
          </div>
        </div>
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      {/* Summary bar */}
      <div className="flex items-center justify-between border-b border-gray-200 bg-white px-4 py-3 dark:border-gray-700 dark:bg-[#090909]">
