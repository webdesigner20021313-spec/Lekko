...
export function RoleEditPage() {
  ...
  useEffect(() => {
    if (!role) { navigate('/users/roles'); return }
    setName(role.name)
    setProjects(
      Object.fromEntries(
        PROJECTS_CONFIG.map((c) => [c.id, mergeProjectAccess(role.projects[c.id], c)])
      )
    )
    setPortalSections(
      Object.fromEntries(
        PORTAL_SECTIONS_CONFIG.map((s) => [s.id, role.portalSections[s.id] ?? buildEmptySectionAccess()])
      )
    )
  }, [role, navigate])
  ...
  function handleSave() {
    ...
    const duplicate = roles.some((r) => r.id !== id && r.name.toLowerCase() === trimmed.toLowerCase())
    ...
    updateRole(id!, { name: trimmed, projects: activeProjects, portalSections: activeSections })
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-white dark:bg-[#090909]">
                ...
                <div key={config.id} className={cn(
                  ...
                  <div className="flex items-center gap-3 px-4 py-3">
                    <div className="flex-1">
                      <p className={cn('text-sm font-semibold transition-colors', proj.enabled ? 'text-gray-900 dark:text-gray-100' : 'text-gray-400')}>
                        {config.label}
                      </p>
      ...
      {/* Sticky footer */}
      <div className="shrink-0 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-[#090909] px-6 py-4 shadow-sm">
