...
import {
  ...
  PERMISSION_LABELS, mergeProjectAccess, buildEmptySectionAccess,
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 backdrop-blur-sm px-4 pt-10 pb-4">
      ...
      >
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-gray-100 px-6 py-4">
          <div>
            <h2 className="text-lg font-bold text-gray-900">Редактирование роли</h2>
            <p className="text-sm text-gray-400">Измените доступы роли</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
          ...
          <div className="flex flex-col gap-3">
                ...
                <div key={config.id} className={cn(
                  ...
                  <div className="flex items-center gap-3 px-4 py-3">
                    <div className="flex-1">
                      <p className={cn('text-sm font-semibold', proj.enabled ? 'text-gray-900' : 'text-gray-400')}>{config.label}</p>
                    ...
                    <button
                      ...
                      className={cn('relative inline-flex h-6 w-11 shrink-0 items-center rounded-full transition-colors duration-200', proj.enabled ? 'bg-gray-900' : 'bg-gray-200')}
                    >
                                ...
                                <td className="px-4 py-2.5">
                                  <div className="flex items-center gap-2">
                                    <button
                                      ...
                                      className={cn('relative inline-flex h-4 w-7 shrink-0 items-center rounded-full transition-colors', acc.enabled ? 'bg-gray-900' : 'bg-gray-200')}
                                    >
                                ...
                                {PERMS.map((perm) => (
                                  <td key={perm} className="px-3 py-2.5 text-center">
                                    <button
                                      ...
                                      className={cn('mx-auto flex h-5 w-5 items-center justify-center rounded border-2 transition-colors', acc[perm] ? 'border-gray-900 bg-gray-900' : 'border-gray-200 bg-white')}
                                    >
                  ...
                  {proj.enabled && config.sections.length === 0 && (
                    <div className="border-t border-gray-100 px-4 py-3 text-xs text-gray-400">Разделы появятся позже</div>
                  )}
                </div>
              )
            })}
            ...
            {PORTAL_SECTIONS_CONFIG.map((sec) => {
              ...
              return (
                <div key={sec.id} className={cn('overflow-hidden rounded-xl border transition-colors', acc.enabled ? 'border-gray-200 bg-white' : 'border-gray-100 bg-gray-50/60')}>
                  <div className="flex items-center gap-3 px-4 py-3">
                    <div className="flex-1">
                      <p className={cn('text-sm font-semibold', acc.enabled ? 'text-gray-900' : 'text-gray-400')}>{sec.label}</p>
                    </div>
                    <button
                      onClick={() => togglePortalEnabled(sec.id)}
                      className={cn('relative inline-flex h-6 w-11 shrink-0 items-center rounded-full transition-colors duration-200', acc.enabled ? 'bg-gray-900' : 'bg-gray-200')}
                    >
